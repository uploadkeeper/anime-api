...
export const scrapeSearch = async ({ list = [], keyw, page = 1 }) => {
  try {
    ...
    $('div.last_episodes > ul > li').each((i, el) => {
      let imgUrl2 = $(el).find('div > a > img').attr('src').split("/").reverse()
      let finalimg = "https://gogocdn.net/" + imgUrl2[1] + "/" +imgUrl2[0]
      list.push({
...
export const scrapeRecentRelease2 = async ({ list = [], page = 1, type = 1 }) => {
  try {
    const mainPage = await axios.get(`
        ${recent_release_url}?page=${page}&type=${type}
        `);
  ...
  try {
    ...
    if (Genres.indexOf(genre) > -1) {
      ...
      $('div.last_episodes > ul > li').each((i, elem) => {
        let imgUrl2 = $(elem).find('div > a > img').attr('src').split("/").reverse()
        let finalimg = "https://gogocdn.net/" + imgUrl2[1] + "/" +imgUrl2[0]
        list.push({
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animeImage = $('div.anime_info_body_bg > img').attr('src').split("/").reverse()
    let finalimg = "https://gogocdn.net/" + animeImage[1] + "/" +animeImage[0]
